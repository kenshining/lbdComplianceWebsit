...
function sendEmail(successFn){
	...
	$.lbdAjax({
		...
		data:{
			...
			content:JSON.stringify({"link":serverUrl+":"+serverPort+"/account/securitySetting/bindEmailResult?state=1"}),
			_csrf:$("#_csrf").val()
		},
...
$(document).ready(function(){
	...
	$('#bind_email_btn').click(function(){
		...
		if(ValidationUtils.isNull(email)){
			...
			$('#emailError_tip').html("<em class='icon icon-er'></em>"+ValidationEmu.errorMsg.bind_new_email_no).show();
