 ...
 */
 ...
 var flicker_fc_move_flicker = null;
 var flicker_fc_auto_flick_reset = null;
 
 function flicker_index(index)
 {
	flicker.settings.flick_position = index;
	flicker_fc_move_flicker();
	flicker_fc_auto_flick_reset();
 }
	...
	Plugin.prototype 					= 
	{
		init 							: function()
		{
			...
			function fc_move_flicker()
			{
				...
				//********************Fixed By S.L************************
				if($settings.dot_outside)
				{
					$settings.dot_outside_func($position);
				}
				$flicker.attr('data-flick-position', $settings.flick_position);
			};
			
			flicker_fc_auto_flick_reset = fc_auto_flick_reset;
			flicker_fc_move_flicker = fc_move_flicker;
		}
	};
